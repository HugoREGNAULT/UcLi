...
async def on_message(message):
    guild = message.guild
    if message.author.id == 1001970528837435403 or message.content.lower() in ["UcLi", "1001970528837435403", '<@1001970528837435403>']:

        embed = discord.Embed(title = '\🚀 uClI est fonctionnel !',
                              description = f'''> Mon **préfixe** est : `/<commande>`
> Ma **latence actuelle** : `{round(bot.latency * 1000)}ms`. \🏓
> `103` **/ commande disponible**.

> » Pour effectuer une commande, faites `/<commande>` en remplaçant `<commande>` par le nom de la commande que vous souhaitez utiliser.
*Ex : </rank:1182046272572625067>*.

• \📌 **Pour plus d'informations sur les commandes, vous pouvez exécuter la commande </help:1180816142030352424>**.''', 
color = 0X151976)
...
#    formatted_time = current_time.strftime("%d %B %Y - %Hh%M")
#
#    embed = discord.Embed(title = "\🚀 Merci de m'avoir ajouté sur votre serveur !",
#                          description = "*Je suis là pour aider et faciliter votre expérience sur Discord.*\n\n> \📌 **N'hésitez pas à me demander de l'aide en utilisant la commande </help:1180816142030352424>.**\n» Si vous avez des questions ou des suggestions, rejoignez notre serveur discord !",
#                          color = 0x1A2B60)
#    
#    embed.add_field(name = "<:PingConnection:1191802986549219430> Ping du Bot :", value = f"» `{round(bot.latency * 1000)}ms`", inline = True)
#    embed.add_field(name = "<:ServerGuide:1191803375830958321> Nombre de Serveurs", value = f"» `{len(bot.guilds)}`", inline = True)
#    embed.add_field(name = "<:GuildsMembers:1191803373897400390> Nombre d'Utilisateurs", value = f"» `{total_members}`", inline = True)
#
#    embed.set_footer(text = f"NoodleBot © 2024 Tous Droits Réservés ・ {formatted_time}")
#
#    await new_channel.send(embed = embed, view = view_linkedbutton())
#
#
...
async def bio():
    ...
    for guild in bot.guilds:
        ...
        k_number = "{:.2f}k".format(total_members / 1000)
    status = [f'{str(len(bot.guilds))} Serveurs', f'{k_number} Utilisateurs', '🎄 Joyeuses Fêtes !', 'V 0.0.1']
    for i in status:
...
async def on_member_join(member):
    ...
    try:
        with open('D:\\Discord\\Projets\\NoodleBot\\V2\\Bot\\guildsconfigurations.json', 'r') as f:
    ...
    autorole_role_id = guild_config.get('autorole_role')
    autorole_logschannel_id = guild_config.get('autorole_logschannel_id')

    if autorole_role_id:
        try:
            role_id = int(autorole_role_id)
        except ValueError:
            return

        role = member.guild.get_role(role_id)
        if role:
            await member.add_roles(role, reason="Nouveau membre a rejoint le serveur.")


    if autorole_logschannel_id:
        logs_channel = member.guild.get_channel(int(autorole_logschannel_id))
        if logs_channel:

            timestamp = datetime.now().timestamp()
            account_created_at = member.created_at.timestamp()
            formatted_timestamp = f"<t:{int(account_created_at)}:F>"

            embed = discord.Embed(description = f'''{member.mention} a rejoint le serveur.''', color = 0X54B244)
            embed.add_field(name = '\🛬 Date d\'arrivée', value = f'<t:{int(timestamp)}:F> ||(<t:{int(timestamp)}:R>)||', inline = False)
            embed.add_field(name = '\📌 Création du compte', value = f'{formatted_timestamp} (<t:{int(account_created_at)}:R>)', inline = False)
            embed.add_field(name = '\📂 IDs', value = f'''```yaml
Nouvel arrivant : {member.id}
Rôle donné : {autorole_role_id}
Serveur ID : {guild_id}
```''', inline = True)

            embed.set_author(name = f'{member.name} ({member.id})', icon_url = f'{member.avatar.url}')
            embed.set_footer(text = f'''📚 Logs » autorole & nouvel arrivant. | {guild.name}''', icon_url = f'{guild.icon.url}')
